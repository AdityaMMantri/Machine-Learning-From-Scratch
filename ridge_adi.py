...
class RidgeRegression:
    ...
    def cost_function(self,X,y):
        ...
        residual = y - y_hat
        cost = (1/n_sample) * np.sum(residual**2) + self.Lambda * np.sum(self.weights**2)
        return cost
    ...
    def gradient_descent(self,X,y):
        ...
        for i in range(self.iterations):
            ...
            gradient_weights = (-2/n_sample) * np.dot(X.T, residual) + 2 * self.Lambda * self.weights
...
ridge_model=RidgeRegression(alpha=alpha, iterations=iterations, Lambda=Lambda, feature_name=feature_names)
ridge_model.gradient_descent(X_train_scaled, y_train.values)

print("\nModel Evaluation on Training Set:")
ridge_model.evalute_model(X_train_scaled, y_train.values)

print("\nModel Evaluation on Test Set:")
ridge_model.evalute_model(X_test_scaled, y_test.values)

y_pred = ridge_model.predict_values(X_test_scaled)
