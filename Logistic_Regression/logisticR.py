...
class BinaryLogisticRegression:
    ...
    def regularization_loss(self):
        if self.regularization is None:
            ...
            return (self._lambda_* np.sum(np.abs(self.weights)))
        elif self.regularization=="l2":
            return (self._lambda_* np.sum(self.weights**2))
        elif self.regularization == "elasticnet":
            l1 = np.sum(np.abs(self.weights))
            l2 = np.sum(self.weights**2)
    ...
    def compute_cost(self, y, y_pred):
        ...
        cost+=self.regularization_loss()/m
        return cost
    
    def regularization_gradient(self,n_sample):
        if self.regularization is None:
            return np.zeros_like(self.weights)
        elif self.regularization=="l1":
            return (self._lambda_ /n_sample) * np.sign(self.weights)
        elif self.regularization=="l2":
            return (2 * self._lambda_ /n_sample) * self.weights
        elif self.regularization=="elasticnet":
            return (self._lambda_/n_sample)*((self.l1_ratio*np.sign(self.weights))+((1-self.l1_ratio)*2*self.weights))
    ...
    def gradient_descent(self,X,y):    
        ...
        for i in range(self.iterations):
            ...
            gradient_weight += self.regularization_gradient(n_sample)
            ...
            if np.linalg.norm(step_size_weight) < threshold and abs(step_size_bias) < threshold:
    ...
    def sigmoid_curve_probability(self,X,y):
        ...
        idx = np.argsort(z)
        plt.plot(z[idx],sigmoid_value[idx])
