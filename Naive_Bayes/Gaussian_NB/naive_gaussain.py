...
class NaiveBayerClassifier:
    ...
    def Gaussian_conditonal_probability(self,X,class_index):
        eps=1e-6

        mean = self.mean[class_index]
        variance = self.variance[class_index]

        exponent=np.exp(-0.5 * ((X-mean)**2) / (variance + eps))
...
df=pd.read_csv(r"F:\ML\Naive-Bayes-Classification-Data.csv")
print(df)
print(df.head())
print(df.describe())
print(df.info())
