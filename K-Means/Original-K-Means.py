...
class KMeansClustering:
    ...
    def fit(self,X):
        ...
        np.random.seed(42)
        random_cluster_index=np.random.choice(self.n_samples,size=self.K,replace=False)
        self.centroids=X[random_cluster_index] # random initilization of centroid points
...
model = KMeansClustering(K=5,iterations=100,tolerance=0.001)
model.fit(X)
